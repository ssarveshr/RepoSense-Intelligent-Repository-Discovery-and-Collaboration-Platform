    ...
    def _ensure_oauth_configured(self) -> None:
        if not settings.github_client_id or not settings.github_client_secret:
            raise GitHubOAuthError(
                "GitHub OAuth is not configured on the server. Please set GITHUB_CLIENT_ID and GITHUB_CLIENT_SECRET in backend/.env file (create an OAuth App at https://github.com/settings/developers with Authorization Callback URL http://localhost:8000/api/github/oauth/callback).",
                400,
            )
        if not settings.github_oauth_redirect_uri:
            raise GitHubOAuthError("GitHub OAuth redirect URI is not configured in backend/.env.", 400)
        if not settings.github_token_encryption_key:
            raise GitHubOAuthError("GitHub token encryption key is not configured in backend/.env.", 400)
